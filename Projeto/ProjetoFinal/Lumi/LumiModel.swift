...
struct remedio: Codable, Hashable{
...
//        CountdownItem(name: "Tarefa 2", duration: 2000, action: { print("Ação realizada para Tarefa 2!") }),
 //       CountdownItem(name: "Tarefa 3", duration: 1500, action: { print("Ação realizada para Tarefa 3!") })
    ...
    func fetch(){
        guard let url = URL(string: "http://192.168.128.237:1880/getRemedios") //URL da API que conterá os dados da Lista de remedios com suas especificacoes
        else{
            return
        }
